...
namespace { constexpr double kMinutesPerDay = 24.0 * 60.0; }
...
int WorldTime::GetDay() const { return static_cast<int>(m_totalMinutes / kMinutesPerDay) + 1; }
int WorldTime::GetHour() const
{
    return static_cast<int>(std::fmod(m_totalMinutes, kMinutesPerDay) / 60.0);
}
int WorldTime::GetMinute() const
{
    return static_cast<int>(std::fmod(m_totalMinutes, kMinutesPerDay)) % 60;
}
