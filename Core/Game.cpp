...
bool Game::Initialize(HWND hwnd)
{
    ...
    {
        Debug::Error("Game::Initialize failed : Cube model load failed");
        return false;
    }

    // The original editor demo referenced an OBJ that was not committed.
    // Keep both editable demo objects by using the tracked cube model.
    if (!m_kennyModel.LoadFromObj(m_renderer.GetDevice(), "Models/cube.obj"))
    {
        Debug::Error("Game::Initialize failed : Editor demo model load failed");
        return false;
    }
    ...
    m_cubeObject.transform.position = { -2.0f, 2.0f, -1.0f };
    m_cubeObject.transform.scale = { 0.5f, 0.5f, 0.5f };
    m_objects.push_back(m_cubeObject);

    m_kennyObject.model = &m_kennyModel;
    m_kennyObject.transform.position = { 2.0f, 2.0f, -1.0f };
    m_kennyObject.transform.scale = { 0.8f, 0.8f, 0.8f };
...
void Game::Draw()
{
    ...
    for (const GameObject& object : m_objects)
    {
        if (object.model)
        {
            m_renderer.DrawModel(*object.model, object.transform, m_camera);
        }
    }
    ...
    for (int z = 0; z < m_gridHeight; ++z)
    {
        for (int x = 0; x < m_gridWidth; ++x)
        {
            ...
            if (road)
            {
                ...
                Transform transform;
                transform.position = GridToWorld(x, z);
                transform.position.y = definition.heightOffset;
                transform.scale = definition.scale;
                m_renderer.DrawModel(
                    GetPlacementModel(definition), transform, m_camera, definition.tint);
            }
    ...
    if (m_buildingCells[selectedIndex] < 0 && m_roadCells[selectedIndex] < 0)
    {
        const PlacementDefinition& definition = GetSelectedDefinition();
        Transform preview;
        preview.position = GridToWorld(m_selectedGridX, m_selectedGridZ);
        preview.position.y = definition.heightOffset +
            (definition.kind == PlacementKind::Road ? 0.02f : 0.0f);
        preview.scale = definition.scale;
        m_renderer.DrawModel(
            GetPlacementModel(definition), preview, m_camera, definition.previewTint);
    }
    ...
    if (m_npcActive)
    {
        ...
        m_renderer.DrawModel(
            m_cubeModel, npcTransform, m_camera,
...
{
    std::wstring title = L"FPS : " +
        std::to_wstring(static_cast<int>(m_timeManager.GetFPS()));
    title += m_renderer.IsVSyncEnabled() ? L" | VSync ON" : L" | VSync OFF";
...
bool Game::TryGetMouseGridCell(int& gridX, int& gridZ) const
{
    ...
    if (std::abs(rayDirection.y) < 0.0001f) return false;
...
void Game::UpdatePlacement()
{
    ...
    bool removeRequested = m_inputManager.IsActionPressed(InputAction::Cancel);
#ifdef ENABLE_EDITOR
    // Right mouse remains reserved for the original editor free camera.
    if (m_inputManager.IsKeyPressed(KeyCode::MouseRight)) removeRequested = false;
#endif
    if (removeRequested)
    {
        if (m_placementMode == PlacementKind::Building)
...
{
    return m_placementCatalog.Get(
        m_placementMode == PlacementKind::Building
            ? m_selectedBuildingDefinition : m_selectedRoadDefinition);
}
...
bool Game::LoadPlacementModels()
{
    for (const PlacementDefinition& definition : m_placementCatalog.GetAll())
    {
        ...
        if (!model->LoadFromObj(m_renderer.GetDevice(), definition.modelPath)) return false;
        if (!definition.texturePath.empty() &&
            !model->LoadTextureOverride(m_renderer.GetDevice(), definition.texturePath)) return false;
...
void Game::DrawPlacementSettings()
{
    ...
    ImGui::Checkbox("Placement Input", &m_enablePlacementInput);
    ...
    if (ImGui::BeginCombo("Type", m_placementCatalog.Get(selectedDefinition).displayName.c_str()))
    {
        for (const int index : availableDefinitions)
        {
            const PlacementDefinition& item = m_placementCatalog.Get(index);
            const bool selected = index == selectedDefinition;
            if (ImGui::Selectable(item.displayName.c_str(), selected)) selectedDefinition = index;
    ...
    if (ImGui::Checkbox("Pause time", &paused)) m_worldTime.SetPaused(paused);
    ...
    ImGui::Separator();
    ImGui::TextUnformatted("Placement ON: Left click / A places objects");
    ImGui::TextUnformatted("Debug remove: Esc / controller B");
    ImGui::TextUnformatted("Right drag: Original free camera");
    ImGui::TextUnformatted("Placement OFF: Safe gizmo and picking mode");
    ImGui::End();
...
{
    if (!m_timeHudLayout.visible) return;
    const ImGuiViewport* viewport = ImGui::GetMainViewport();
    ImVec2 anchor = viewport->WorkPos;
    switch (m_timeHudLayout.anchor)
    {
    case HudAnchor::TopRight: anchor.x += viewport->WorkSize.x; break;
    case HudAnchor::BottomLeft: anchor.y += viewport->WorkSize.y; break;
    case HudAnchor::BottomRight:
        anchor.x += viewport->WorkSize.x; anchor.y += viewport->WorkSize.y; break;
    case HudAnchor::Center:
        anchor.x += viewport->WorkSize.x * 0.5f;
        anchor.y += viewport->WorkSize.y * 0.5f; break;
    default: break;
    }
    ...
    ImGui::SetNextWindowSize(
        { m_timeHudLayout.size.x, m_timeHudLayout.size.y }, ImGuiCond_Always);
...
void Game::RebuildNpcPath()
{
    m_npcPath.clear();
    m_npcPathIndex = 0;
    if (!m_npcActive || m_roadCells[CellIndex(m_npcGridX, m_npcGridZ)] < 0) return;
    ...
    while (!open.empty())
    {
        ...
        if (distance[current] > distance[destination]) destination = current;
        for (const auto& direction : directions)
        {
            ...
            if (m_roadCells[next] < 0 || distance[next] >= 0) continue;
    ...
    for (int cell = destination; cell != start; cell = previous[cell]) reversed.push_back(cell);
    m_npcPath.assign(reversed.rbegin(), reversed.rend());
}

void Game::UpdateNpc()
{
    if (!m_npcActive || m_npcGridX < 0 || m_npcGridX >= m_gridWidth ||
